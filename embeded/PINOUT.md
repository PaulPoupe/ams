...
SD-карта сейчас используется как простой raw circular buffer, а не как FAT-файловая система. Прошивка пишет последние 10 секунд `PCM16 mono 16 kHz` в фиксированную тестовую область карты и постоянно перезаписывает её по кругу. Этот слой изолирован в `sd_card_buffer` и позже должен быть заменён на нормальный FatFs-backed storage.
